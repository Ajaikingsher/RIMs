...
import { X, Loader2, Save, Landmark } from "lucide-react"
import { createMilestone, updateMilestone } from "@/lib/supabase/actions"
...
export default function AdminMilestonesForm({ initialData, onClose }: { initialData?: any, onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (initialData?.id) {
        ...
        toast.success("Slideshow milestone updated")
      } else {
        await createMilestone(formData)
        toast.success("Slideshow milestone added")
      }
      onClose()
      window.location.reload()
    } catch (error) {
      console.error(error)
      toast.error("Failed to save milestone slide")
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-heading font-bold">
              {initialData ? "Refine Slide" : "Add New Slide"}
            </h2>
            <p className="text-xs text-white/60">Configure the slide details displayed on the homepage.</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2 space-y-2">
                ...
                <Input 
                  ...
                  required 
                  placeholder="e.g. Official Software Release @ Coimbatore"
            ...
            <div className="space-y-2">
              <Label htmlFor="dateString">Milestone Date (e.g. May 11, 2009)</Label>
              <Input 
                id="dateString" 
                value={formData.dateString} 
                onChange={(e) => setFormData(prev => ({...prev, dateString: e.target.value}))} 
                placeholder="e.g. May 11, 2009 or July 2024"
                className="rounded-xl h-12"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea 
                ...
                required
                placeholder="We were honored to have Hon. Minister Thangam Thennarasu officially launch our flagship enterprise software..."
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100">
              <Label className="mb-4 block text-center font-bold text-gray-700">Slide Image & Focus Positioning</Label>
                ...
                {formData.image && (
                  <div className="w-full space-y-4">
                    <div className="flex justify-between items-center px-1">
                      <Label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Vertical Alignment</Label>
                    ...
                    <p className="text-[10px] text-center text-gray-400 italic">Drag the slider to adjust image center alignment on the homepage.</p>
                  </div>
                )}
              </div>
            </div>
